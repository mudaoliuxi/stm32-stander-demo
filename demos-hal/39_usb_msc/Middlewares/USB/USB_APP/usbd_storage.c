...
/* 文件系统在外部FLASH的起始地址
 * 2240 * 4096是怎么来的, 请看FATFS实验, diskio.c的说明
 */
#define USB_STORAGE_FLASH_BASE  2240*4096
...
int8_t STORAGE_GetCapacity (uint8_t lun, uint32_t *block_num, uint16_t *block_size)
{
    HAL_SD_CardInfoTypeDef sd_card_info_handle;
    switch (lun)
    {
        case 0: /* SPI FLASH */
            *block_size = 512;
            *block_num = (1024 * 7424) / 512;   /* SPI FLASH的7424KB字节,文件系统用 */
            break;

        case 1: /* SD卡 */
            HAL_SD_GetCardInfo(&g_sd_handle, &sd_card_info_handle);
            *block_num = sd_card_info_handle.LogBlockNbr - 1;
            *block_size = sd_card_info_handle.LogBlockSize;
            break;
    }
...
int8_t STORAGE_Read (uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
    ...
    {
        case 0: /* SPI FLASH */
            norflash_ex_read(buf, USB_STORAGE_FLASH_BASE + blk_addr * 512, blk_len * 512);
            break;
...
int8_t STORAGE_Write (uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
    ...
    {
        case 0: /* SPI FLASH */
            norflash_ex_write(buf, USB_STORAGE_FLASH_BASE + blk_addr * 512, blk_len * 512);
            break;
...
int8_t STORAGE_GetMaxLun (void)
{
    ...
    if (sd_card_info_handle.LogBlockNbr)    /* 如果SD卡正常, 则支持2个磁盘 */
    {
