...
void adc_temperature_init(void)
{
    ...
    /* 使能时钟 */
    ADC_ADCX_CHY_CLK_ENABLE();                                                          /* 使能ADC3时钟 */
    rcc_periph_clk_struct.PeriphClockSelection = RCC_PERIPHCLK_ADC;                     /* 配置拓展时钟 */
    rcc_periph_clk_struct.AdcClockSelection = RCC_ADCCLKSOURCE_CLKP;                    /* 配置ADC时钟源 */
    HAL_RCCEx_PeriphCLKConfig(&rcc_periph_clk_struct);
    
    /* 配置ADC */
    g_adc3_handle.Instance = ADC_ADCX;                                                  /* ADC3 */
    ...
    HAL_ADC_Init(&g_adc3_handle);                                                       /* 初始化ADC外设 */
...
short adc3_get_temperature(void)
{
    ...
    adcx = adc3_get_result_average(g_adc3_handle, ADC_ADCX_CHY, 10);        /* 读取内部温度传感器通道,10次取平均 */
