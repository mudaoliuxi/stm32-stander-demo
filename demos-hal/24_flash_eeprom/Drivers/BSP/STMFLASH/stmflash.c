...
void stmflash_write(uint32_t waddr, uint32_t *pbuf, uint32_t length)
{
    ...
    if (waddr < (STM32_FLASH_BASE + BOOT_FLASH_SIZE))
    {
        return;                                                                 /* 写入地址小于STM32_FLASH_BASE+BOOT_FLASH_SIZE,非法. */
    }
    
    if (waddr > (STM32_FLASH_BASE + STM32_FLASH_SIZE))
    {
        return;                                                                 /* 写入地址大于STM32总FLASH地址范围,非法. */
    }
    
    if (waddr % 32)
    {
        return;                                                                 /* 写入地址不是32字节倍数,非法. */
    }
