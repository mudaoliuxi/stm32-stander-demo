...
static uint16_t kbdxsize;  /* 虚拟键盘按键宽度 */
static uint16_t kbdysize;  /* 虚拟键盘按键高度 */
...
RESTART:
    ...
    while (1)
    {
        ...
        if (pykey != 0)                                                             /* 有按键按下 */
        {
            ...
            lcd_show_num(30 + 144, 105, result_num, 1, 16, BLUE);                   /* 显示匹配结果 */
            text_show_string(30 + 40, 105, 200, 16, (char *)inputstr, 16, 0, BLUE); /* 显示有效的字符串 */
