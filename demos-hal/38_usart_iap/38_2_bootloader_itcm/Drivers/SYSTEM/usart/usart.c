...
uint8_t g_usart_rx_buf[USART_REC_LEN] __attribute__ ((at(0x24001000)));
#else                               /* 是AC6编译器，使用AC6编译时 */
uint8_t g_usart_rx_buf[USART_REC_LEN] __attribute__((section(".bss.ARM.__at_0x24001000")));
#endif
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    if(huart->Instance == USART1)                                   /* 如果是串口1，进行串口1 MSP初始化 */
    {
        ...
        HAL_NVIC_SetPriority(USART_UX_IRQn, 0, 0);                  /* 抢占优先级0，子优先级0 */
#endif
    }
}
...
{ 
    ...
    {
        HAL_UART_Receive(&g_uart1_handle, &rxdata, 1, 1000);
