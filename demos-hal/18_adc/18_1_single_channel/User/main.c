...
int main(void)
{
    ...
    while (1)
    {
        adcdata = adc_get_result_average(ADC_ADCX_CHY, 10);     /* 获取通道19的转换值，10次取平均 */
        lcd_show_xnum(142, 110, adcdata, 5, 16, 0, BLUE);       /* 显示ADC采样后的原始值 */
        
        voltage = (float)adcdata * (3.3 / 65536);               /* 获取计算后的带小数的实际电压值 */
        adcdata = voltage;                                      /* 赋值整数部分给adcx变量 */
        lcd_show_xnum(142, 130, adcdata, 1, 16, 0, BLUE);       /* 显示电压值的整数部分 */
        
        voltage -= adcdata;                                     /* 把已经显示的整数部分去掉，留下小数部分 */
        voltage *= 1000;                                        /* 小数部分乘以1000 */
        lcd_show_xnum(158, 130, voltage, 3, 16, 0X80, BLUE);    /* 显示小数部分（前面转换为了整形显示），这里显示的就是111. */
