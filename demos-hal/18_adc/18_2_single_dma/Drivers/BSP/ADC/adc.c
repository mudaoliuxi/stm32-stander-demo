...
void adc_dma_init(uint32_t par, uint32_t mar)
{
    ...
    g_adc_dma_handle.Init.NbrOfConversion = 1;                                          /* 赋值范围是1~16，本实验用到1个通道 */
    g_adc_dma_handle.Init.DiscontinuousConvMode = DISABLE;                              /* 禁止常规转换组不连续采样模式 */
    g_adc_dma_handle.Init.NbrOfDiscConversion = 0;                                      /* 配置不连续采样模式的通道数，禁止常规转换组不连续采样模式后，此参数忽略 */
    ...
    g_adc_dma_handle.Init.Overrun = ADC_OVR_DATA_OVERWRITTEN;                           /* 有新的数据后直接覆盖掉旧数据 */
