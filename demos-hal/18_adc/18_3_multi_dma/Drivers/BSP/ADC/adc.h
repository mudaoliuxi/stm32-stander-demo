...
#define ADC_ADCX_DMASx_IS_TC()          ( __HAL_DMA_GET_FLAG(&g_dma_nch_adc_handle, DMA_FLAG_TCIF3_7) )                /* 获取DMA1 Stream7传输完成标志位，
                                                                                                                        * 这是一个假函数形式，不能当函数使用
                                                                                                                        */
#define ADC_ADCX_DMASx_CLR_TC()         do{ __HAL_DMA_CLEAR_FLAG(&g_dma_nch_adc_handle, DMA_FLAG_TCIF3_7); }while(0)   /* 清除DMA1 Stream7传输完成标志位 */

/* 函数声明 */
void adc_dma_enable( uint16_t ndtr);                /* 使能一次DMA传输ADC数据 */
void adc_nch_dma_init(uint32_t par, uint32_t mar);  /* 初始化ADC多通道和DMA */
