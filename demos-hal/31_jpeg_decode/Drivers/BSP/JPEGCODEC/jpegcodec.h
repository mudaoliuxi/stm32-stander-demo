...
/* 函数声明 */
void jpeg_dma_stop(void);                                                           /* 停止JPEG MDMA解码过程 */
void jpeg_in_dma_start(void);                                                       /* 启动jpeg in mdma,开始解码JPEG */
void jpeg_out_dma_start(void);                                                      /* 启动jpeg out mdma,开始输出YUV数据 */
void jpeg_in_dma_resume(uint32_t memaddr, uint32_t memlen);                         /* 恢复MDMA IN过程 */
void jpeg_out_dma_resume(uint32_t memaddr, uint32_t memlen);                        /* 恢复MDMA OUT过程 */
void jpeg_in_dma_init(uint32_t meminaddr, uint32_t meminsize);                      /* JPEG硬件解码输入MDMA配置 */
void jpeg_out_dma_init(uint32_t memoutaddr, uint32_t memoutsize);                   /* JPEG硬件解码输出MDMA配置 */
uint8_t jpeg_get_quality(void);                                                     /* 得到JPEG图像质量 */
void jpeg_get_info(jpeg_codec_typedef *tjpeg);                                      /* 获取图像信息 */
void jpeg_decode_init(jpeg_codec_typedef *tjpeg);                                   /* 初始化硬件JPEG解码器 */
void jpeg_core_destroy(jpeg_codec_typedef *tjpeg);                                  /* 关闭硬件JPEG内核,并释放内存 */
uint8_t jpeg_core_init(jpeg_codec_typedef *tjpeg);                                  /* 初始化硬件JPEG内核 */
uint8_t jpeg_dma2d_yuv2rgb_conversion(jpeg_codec_typedef *tjpeg, uint32_t *pdst);   /* 将YUV数据转换成RGB数据 */
