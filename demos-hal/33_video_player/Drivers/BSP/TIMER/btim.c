...
extern uint8_t g_frame;                 /* videoplayer.c里面定义 */
extern uint8_t g_frameup;               /* videoplayer.c里面定义 */
...
void btim_tim6_int_init(uint16_t arr, uint16_t psc)
{
    ...
    /* 配置基本定时器 */
    g_timx_handle.Instance = BTIM_TIM6_INT;                 /* 基本定时器6 */
    ...
    /* 使能基本定时器及其相关中断 */
    HAL_NVIC_SetPriority(BTIM_TIM6_INT_IRQn, 1, 0);         /* 设置中断优先级，抢占优先级1，子优先级0 */
...
void btim_tim7_int_init(uint16_t arr, uint16_t psc)
{
    ...
    /* 配置基本定时器 */
    g_tim7_handle.Instance = BTIM_TIM7_INT;                 /* 基本定时器7 */
    ...
    /* 使能基本定时器及其相关中断 */
    HAL_NVIC_SetPriority(BTIM_TIM7_INT_IRQn, 1, 0);         /* 设置中断优先级，抢占优先级1，子优先级0 */
...
{
    if (__HAL_TIM_GET_FLAG(&g_tim7_handle, TIM_FLAG_UPDATE) == SET) /* 判断跟新中断标志 */
    {
        g_frameup = 1;
        LED1_TOGGLE();
        __HAL_TIM_CLEAR_IT(&g_tim7_handle, TIM_IT_UPDATE);          /* 清除更新中断标志 */
    }
}
