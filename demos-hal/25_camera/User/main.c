...
void jpeg_test(void)
{
    ...
    lcd_show_string(30, 140, 200, 16, 16, "KEY0:Effects", RED);                                                                         /* 特效 */
    lcd_show_string(30, 160, 200, 16, 16, "WK_UP:Size", RED);                                                                           /* 分辨率设置 */
            ...
            if (key)                                                                                                                    /* 有按键按下,需要处理 */
            {
                ...
                switch (key)
                {
                    ...
                    case WKUP_PRES:                                                                                                     /* JPEG输出尺寸设置 */
                    {
                        ...
                        if (size > 5)
                        {
                            size = 0;                                                                                                   /* 最大只支持WXGA的jpeg数据保存，再大分辨率就不够内存用了 */
                        }
...
int main(void)
{
    ...
    lcd_init();                                 /* 初始化LCD */
    key_init();                                 /* 初始化按键 */
    btim_timx_int_init(10000 - 1, 24000 - 1);   /* 10Khz计数, 1秒钟中断一次, 用于统计帧率 */
    ...
    if (g_ov_mode == 1)
    {
        jpeg_test();                                                                /* RGB565模式测试 */
    }
    else 
    {
        rgb565_test();                                                              /* JPEG模式测试 */
    }
}
