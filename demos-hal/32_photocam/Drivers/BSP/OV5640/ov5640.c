...
uint8_t ov5640_write_reg(uint16_t reg, uint8_t data)
{
    ...
    if (sccb_send_byte(OV5640_ADDR))
    {
        res = 1;    /* 写器件ID */
    }
    
    if (sccb_send_byte(reg >> 8))
    {
        res = 1;    /* 写寄存器高8位地址 */
    }
...
/* 特效设置参数表,支持7个特效 */
const static uint8_t OV5640_EFFECTS_TBL[6][3] =
{
    0X06, 0x40, 0X10, /* 正常 */
    0X1E, 0xA0, 0X40, /* 冷色 */
    0X1E, 0x80, 0XC0, /* 暖色 */
