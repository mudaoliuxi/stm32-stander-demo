...
void dac_set_voltage(uint8_t outx, uint16_t vol)
{
    ...
    if (dacdata >= 4096)
    {
        dacdata = 4095;                                                             /* 如果值大于等于4096,则取4095 */
    }
    
    if (outx == 1)                                                                  /* 通道1 */
    {
        HAL_DAC_SetValue(&g_dac_handle, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dacdata);   /* 12位右对齐数据格式设置DAC值 */
    }
    else                                                                            /* 通道2 */
    {
        HAL_DAC_SetValue(&g_dac_handle, DAC_CHANNEL_2, DAC_ALIGN_12B_R, dacdata);   /* 12位右对齐数据格式设置DAC值 */
    }
}
