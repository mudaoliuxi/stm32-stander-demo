...
int main(void)
{
    ...
    lcd_show_string(30, 130, 200, 16, 16, "WK_UP:AUTO/MANUL", RED);
    delay_ms(1200);
    ...
    while (1)
    {
        ...
        BTIM_TIMX_INT->CNT = 0;                                                                                 /* 重设基本定时器的计数器值 */
        g_timeout = 0;
        
        julia_generate_fpu(lcddev.width, lcddev.height, lcddev.width / 2, lcddev.height / 2, zoom_ratio[i]);    /* 产生Julia分形图形 */
        
        time = BTIM_TIMX_INT->CNT + (uint32_t)g_timeout * 0x10000;                                              /* 计算耗时 */
